...
if (isset($_POST['productaddbtn'])) {
    $product_id = str_pad(rand(0, 9999999999), 10, '0', STR_PAD_LEFT);
    ...
    if (file_exists("upload/" . $_FILES["product_image"]['name'])) {
        ...
        $query = "INSERT INTO `producttb` (product_id, productname, productdesignation, productdescription, productimage, category_id)
                  VALUES ('$product_id', '$product_name', '$product_designation', '$product_description', '$product_image', '$categort')";
        $query_run = mysqli_query($connection, $query);
